...
document.addEventListener('DOMContentLoaded', () => {
  const path = window.location.pathname; // ejemplo: /products/index/category/BEBIDAS
  const partes = path.split('/');
  const indexCategoria = partes.indexOf('category');
  if (indexCategoria !== -1 && partes[indexCategoria + 1]) {
    const categoriaURL = decodeURIComponent(partes[indexCategoria + 1]).toUpperCase();
    const checkbox = Array.from(document.querySelectorAll('input[name="categorias[]"]')).find(
      cb => cb.value.toUpperCase() === categoriaURL
    );

    if (checkbox) {
      checkbox.checked = true;
      categoriasSeleccionadasGlobal = categoriaURL; // opcional, por consistencia
    }
  }
});
...
function filtrado(eslimpieza = false) {
  ...
  paginateProducts(categoriasSeleccionadasGlobal, precioMinGlobal, precioMaxGlobal, sortGlobal, eslimpieza);
}
function paginateProducts(categoriasSeleccionadas = '', precioMin = 0, precioMax = 999999, orden = '', eslimpieza,) {
  loading = true;
  document.getElementById('loading').classList.remove('hidden');

  const currentUrl = window.location.pathname.split('/');
  const campoExtra = currentUrl[3] || '';
  const valorExtra = currentUrl[4] || '';
  ...
  console.log(seLimpio);
  if (eslimpieza && seLimpio == 0) {
    seLimpio = 1;
  }
  query += `&limpiar=${seLimpio}&campoExtra=${campoExtra}&valorExtra=${valorExtra}`;
  console.log(seLimpio);


  fetch(`/products/paginate_Products${query}`)
    .then(res => res.json())
    .then(data => {
...
function reloadProducts(page = 1) {
  ...
  fetch(`/products/paginate_Products?page=${page}&sort=${sort}`)
    .then(res => res.json())
    .then(data => {
      const grid = document.getElementById("product-grid");
      grid.innerHTML = "";
