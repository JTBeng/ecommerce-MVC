...
if ($cant != 0):
?>
    <div class="lg:sticky lg:top-20 md:rounded-xl fixed bottom-0 left-0 right-0 md:static z-10 bg-background-light/80 p-4 backdrop-blur-sm ">
        ...
        <button class="cursor-pointer block mx-auto mt-4 py-2 w-[92%] rounded-xl bg-gradient-to-r from<?= $config['color_principal'] ?> to<?= $config['color_secundario'] ?> text-white font-semibold text-center shadow hover:scale-[1.03] transition-transform duration-300">
            Proceder al Pago
        </button>
    </div>
<?php endif; ?>